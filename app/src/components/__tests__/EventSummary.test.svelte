...
{#if event.category}
  <div class="mb-4 h-6 border border-black bg-transparent dark:bg-zinc-800">
    {event.category}
  </div>
{/if}
...
<div class="flex min-h-[60px] flex-col gap-5 pb-6 sm:flex-row">
  <div class="w-full sm:min-h-60 sm:w-[40%]">
    <!-- Simplified EventInfoBox -->
    <div data-testid="event-info-box" data-props={JSON.stringify({ event, numberOfParticipants: data.numberOfParticipants })}></div>
  </div>
  {#if event.image}
...
{#if event.body}
  <div class="portable-text flex flex-col gap-4 text-base sm:w-[60%]">
    <!-- Simplified PortableText -->
    <div data-testid="portable-text" data-props={JSON.stringify({ value: event.body, components: { marks: { link: 'Link' } } })}></div>
  </div>
{/if}
