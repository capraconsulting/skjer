...
test("handles network errors gracefully", async ({ page, context }) => {
  ...
  console.log(
    "Normal page content (first 200 chars):",
    normalPageContent.substring(0, 200) + "..."
  );
  ...
  console.log(
    `Content length difference: ${(pageContent?.length ?? 0) - normalPageContent.length}`
  );
  ...
  const missingApiContent = apiLoadedContent.filter(
    (content) =>
  ...
  console.log(
    `Missing API content: ${missingApiContent.length > 0 ? missingApiContent.join(", ") : "None"}`
  );
...
test("handles slow network gracefully", async ({ page }) => {
  // Simuler treg nettverksforbindelse
  await page.route("**/*", async (route) => {
    await new Promise((resolve) => setTimeout(resolve, 2000)); // 2 sek forsinkelse
    await route.continue();
  });

  await page.goto("/");

  // Gjør lastindikatoren synlig for testen
  await page.evaluate(() => {
  ...
  // Legg til eksplisitt tidsavbrudd for lasteindikatoren
