...
{:else}
  {#if $isPreviewing}
    ...
    >
      <span class="block group-hover:hidden">{$_t('common.previewOn')}</span>
      <span class="hidden group-hover:block">{$_t('common.previewOff')}</span>
    </a>
    <span
    ...
    >
      <span>{$_t('common.previewRequiresPublished')}</span>
    </span>
  {/if}
